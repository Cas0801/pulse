  ...
  return (
    <div className="min-h-screen bg-transparent">
      <header className="sticky top-0 z-40 border-b border-line/70 bg-white/88 px-5 pb-4 pt-4 backdrop-blur-xl lg:px-7">
        <div className="section-label">发现</div>
      ...
      <main className="px-4 pb-28 pt-5 lg:px-7 lg:pb-10">
        <section className="grid gap-4 lg:grid-cols-[1.2fr_0.8fr]">
          <div className="ios-card overflow-hidden rounded-[28px]">
            ...
            <div className="p-6">
              ...
              <p className="mt-3 max-w-xl text-sm leading-6 text-ink/60">精选近期热门作品。</p>
            </div>
          </div>
          ...
          <div className="space-y-4">
            ...
            <div className="ios-card rounded-[28px] p-5">
              ...
              <div className="mt-4 space-y-3 text-sm text-ink/62">
                <div className="ios-panel rounded-[20px] px-4 py-3">按分类、关键词和标签快速查找内容。</div>
                <div className="ios-panel rounded-[20px] px-4 py-3">查看近期值得关注的主题和作品。</div>
              </div>
            </div>
          </div>
        </section>
            ...
            {discover.categories.map((cat) => (
              <button
                ...
                className={`shrink-0 rounded-full px-4 py-2.5 text-[12px] font-medium transition-colors ${
                  selectedCategory === cat ? 'bg-accent text-white shadow-[0_10px_20px_rgba(16,163,127,0.18)]' : 'ios-pill text-ink/65 hover:text-ink'
                }`}
              >
                {cat}
              </button>
        ...
        <section className="mt-7">
          <div className="mb-4 flex items-end justify-between gap-4 px-1">
            <div>
              <div className="section-label">内容列表</div>
              <h3 className="mt-1 text-[24px] font-semibold text-ink">精选内容</h3>
              <p className="mt-1 text-[13px] text-ink/52">按分类查看推荐内容。</p>
            </div>
            <button
