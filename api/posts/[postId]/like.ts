...
async function fetchLikeCount(postId: string, accessToken?: string) {
  const rows = await fetchJson<Array<{ likes_count: number | null }>>(
    `${getRestBaseUrl()}/posts?id=eq.${encodeURIComponent(postId)}&select=likes_count&limit=1`,
    undefined,
    accessToken,
  );
...
export default async function handler(req: any, res: any) {
  ...
  try {
    if (!hasSupabaseConfig()) {
      const payload: LikeResult = {
        postId: req.query.postId,
        liked: req.method === 'POST',
        likes: req.method === 'POST' ? 1 : 0,
      };
    ...
    if (!actorId || !postId) {
      res.status(400).json({ message: '缺少 postId 或用户身份，无法执行点赞操作' });
      return;
    }
    ...
    try {
      if (req.method === 'POST') {
        await fetchJson<unknown>(
          `${getRestBaseUrl()}/post_likes?on_conflict=post_id,user_id`,
          {
            method: 'POST',
            headers: {
        ...
        await fetchJson<unknown>(
          `${getRestBaseUrl()}/post_likes?post_id=eq.${encodeURIComponent(postId)}&user_id=eq.${encodeURIComponent(actorId)}`,
          {
            method: 'DELETE',
            headers: {
    ...
    res.status(200).json({
      postId,
      liked: req.method === 'POST',
      likes: await fetchLikeCount(postId, accessToken),
    });
  } catch (error) {
    res.status(500).json({
      message: req.method === 'POST' ? '点赞失败' : '取消点赞失败',
